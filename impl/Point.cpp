...
Point Point::dummyPoint(){  // TODO sounds like bad practice - will the user/alg know the encrypted point equals zero?
    const vector<Binary> &zeroVector = vector<Binary>(DIM, 0);
    return Point(zeroVector);
}

Point::Point(std::vector<Binary> coordinates) : coordinates(std::move(coordinates)) {}
...
Point operator / (const Point &p, const Binary &b) {
    //todo consider std::transform(myv1.begin(), myv1.end(), myv1.begin(),std::bind1st(std::multiplies<T>(), 3));
    vector<Binary> coorVector;
    for (const Binary &coor : p.getCoordinates()){
