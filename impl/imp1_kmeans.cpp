...
//            EncNumber amount = EncNumber(0);
            Bit amount = keysServer.randomBit(); //= 0;  // fixme either a security issue or precision
            ...
            for(const Point & p : currStrip) {
                ...
                for(const Point & r2 : random) {
                    Bit r2smaller = cmp(p, r2);
                    r2smaller.multiplyBy(cmp(R, r2));  // r2smaller = (p > r2) * (R > r2)
                    
                    Bit r2bigger = cmp(r2, R);  // r2bigger = (r2 > R)
//                    Bit r2bigger = cmp(r2, p) * cmp(r2, R); //this worked but was redundant
//
//                  Bit isAboveR2 = r2smaller + r2bigger - r2smaller * r2bigger
                    Bit isAboveR2 = r2smaller;
                    isAboveR2.multiplyBy(r2bigger);
                    isAboveR2.negate();
                    isAboveR2 += r2smaller;
                    isAboveR2 += r2bigger;
...
//                Bit isAboveOtherReps = accumulate(cmpIsAbove.begin(), cmpIsAbove.end(), 1, multiplies<Bit>());
            ...
            if(dbg) {
                sum = sum - currStrip[0]; //todo consider removing line for efficiency
...
/*vector<vector<Point> > getLeftoverPoints(const vector<Point> & points, const vector<Point> & means, KeysServer & keysServer) {
    vector<DecryptedPoint> decMeans = sk.decryptPointsByCA(
    ...
    vector<EncNumber> distances = getDistFromClosestMeanByClient(decMeans, points, sk);
