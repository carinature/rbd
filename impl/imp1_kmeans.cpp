...
void encryptedKmeans() {
  ...
  vector<DecryptedPoint> means = calculateMeans(
          getCells(points, *ks), *ks); // convert into eps-net and return a list of the cell means
...
//            Point sum = R;
      ...
      for (const Point & p : currStrip) { // for every point match it's corresponding
        ...
        for (const Point & r2 : random) {
          ...
          cmpIsAbove.push_back(isAboveR2);  // currently the vector is for DBG purposes - cmpIsAbove *= isAboveR2;
        }
        ...
        Point isPoint = p * isInCell;  // equals org point if point is included in the cell, and null(0) point otherwise
        ...
        sizeVec.append(isInCellNum);  // more efficient to sum all the bits together, which we are doing in a later step
        if (DBG) {
  ...
  if (DBG) {
    decAndWriteToFile(randPoints, "io/random_means", keysServer);
