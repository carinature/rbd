...
vector<vector<Point> >
getLeftoverPoints( const vector<PointExtended> & clients,
...
//    randPoints.insert(randPoints.end(), copy.begin(), copy.end());  // <--- this is an experimental line, to cover the cases of top strip points
        ...
        printDuration( t1, "Dict Creation" );
        //    CUT INTO CELLS
...
//          fcout << "strip:" << i << ", rp: " << rp << ", p: " << j++ << " ----- "; // << endl;   //DBG
                ...
                isInCell.multiplyBy(
                        isAboveOtherReps );   //  is point in cell = is under random * is over other randoms
                Point isPoint =
...
//          fcout << "   isInCellNums: " << keysServer.decrypt(isInCellNum) << " ";
    ...
    if ( DBG ) {
        ...
        writeToFile( badRands, rands_bad_file );//, keysServer);
