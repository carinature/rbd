...
int main(int argc, char *argv[]) {
    ...
    //in the future should be FHEPubKey * pubKey = ks->pubKey;
    //
...
/**
 * encryptedKmeans - receives a set of data points and converts into a reduced set of data points,
...
void encryptedKmeans() {
    ...
    //choose the closest points to the means for the next step of the algorithm
    //
    vector<Point> chosen = pointsSplited[0];

    //remove and save the furthest points of the set for the next iteration
    //
    ...
    for (const DecryptedPoint &point : chosenForCorset) {
        vector<double> dpd;
        for (long coor : point) {
            dpd.push_back(coor / CONVERSION_FACTOR);
        }
        dps.push_back(dpd); 
        //if (0==dp[0] && 0==dp[1]) ++count;  // for DBG
    }
...
/**
 * getCells - receives a vector of points and breaks it into a list of cells (group of close points)
 * @points - vector of points / clients
...
//    randPoints.insert(randPoints.end(), copy.end(), copy.end());  // <--- this is an experimental line, to cover the cases of top strip points
        ...
        //CUT INTO CELLS
        //
        //for each randomly chosen point (meaning for each potential cell)
...
//          Point sum = R;
            ...
            //curr sell size is zero
            //
            Vec<EncNumber> sizeVec;  // vector<Bit> sizeVec;
            //for each point in the strip
            //
            // for every point match it's corresponding #### is above other randomly chosen points in the strip
            //
            for (const Point &p : currStrip) { 
                
                //find out if the current point is above all the random points which are below the current one
                //
...
//          fcout << "strip:" << i << ", rp: " << rp << ", p: " << j++ << " ----- "; // << endl;   //DBG
                ...
                for (const Point &r2 : random) {
                    ...
                    //currently the vector is for DBG purposes - cmpIsAbove *= isAboveR2;
                    //
                    cmpIsAbove.push_back(isAboveR2);
                }
                Bit isAboveOtherReps = cmpIsAbove[0];
                //accumulate prod of cmpIsAbove
                //
                for (const Ctxt& b : cmpIsAbove) {
                    isAboveOtherReps.multiplyBy(b);
                }
                Bit isInCell = cmp[R][p][0];
                
                //is point in cell = is under random * is over other randoms
                //
                isInCell.multiplyBy(isAboveOtherReps);
                
                //equals org point if point is included in the cell, and null(0) point otherwise
                //
                Point isPoint = p * isInCell;
                
                //more efficient to sum all the points together
                //
...
//          fcout << "   isInCellNums: " << keysServer.decrypt(isInCellNum) << " ";
                ...
                //more efficient to sum all the bits together, which we are doing in a later step
                //
                sizeVec.append(isInCellNum);
                if (DBG) {
...
/**
 * calculateMeans - uses calculateCellMean for a list cell and their sizes to find for each cell its corresponding mean
...
/** 
 * calculateCellMean - receives group of points (cell) and returns their mean.
 * calculate the sum and then uses a server to make the final avg ccalculation (sum/size)
...
DecryptedPoint calculateCellMean(tuple<Cell, EncNumber> cellTuple, KeysServer &keysServer) {
    ...
    //array of vectors - cell per coordinate - vector of encrypted numbers
    //
    Vec<EncNumber> vec[DIM];
    ...
    EncNumber sum[DIM];
    for (int j = 0; j < DIM; ++j) {
        sum[j] = sumEncNumVec(vec[j], NUM_POINTS * pow(EPSILON, 2));
    }
    Point sumPoint = Point(vector<EncNumber>(sum, sum + DIM), &keysServer);
    //calculate the avg point using the server
    //
...
/**
 * getDistances - receives a list of clients and returns the shortest distance between the client and it's closest mean
...
/**
 * calculateThreshold - receives a list of distances and returns the average distance,
...
/**
 * getLeftoverPoints - split the group of points into "close" and "far" points.
